...
// ASM search, but also ensuring that the rect is in the image boundaries.
    ...
    static DetectorParameter ImgDetParToRoiFrame(
            const DetectorParameter &detpar,          // in
            const Rect &rect_roi)        // in
    {
        DetectorParameter detpar_roi(detpar);
...
// to undo the effects of searching on the ROI, not on the actual image.
    ...
    Shape RoiShapeToImgFrame(     // return shape in image frame
            ...
            const DetectorParameter &detpar_roi, // in: detpar wrt the ROI
            const DetectorParameter &detpar)     // in: detpar wrt the image
    {
    ...
    void FaceRoiAndDetectorParameter(        // get ROI around the face, rotate if necessary
            Image &face_roi,   // out
            DetectorParameter &detpar_roi, // out: detpar wrt the ROI
            const Image &img,        // in: original image
            const DetectorParameter &detpar,     // in: wrt img frame, only x,y,w,h,rot used
            bool flip,       // in: mirror the ROI?
