...
// center of the mouth determined by manual landmarking).  Correct that here.
    ...
    static void TraceEyeMouthImg(
        CImage& cimg,             // in
        DetectorParameter& detpar,// in
        int     ileft_best,       // in
        int     iright_best,      // in
        int     imouth_best)      // in
    {
        cv::rectangle(cimg, // border color shows eyaw
    ...
    void DetectEyesAndMouth(  // use OpenCV detectors to find the eyes and mouth
            DetectorParameter &detpar, // io: eye and mouth fields updated, other fields untouched
            const Image &image)   // in: ROI around face (already rotated if necessary)
    {
#if TRACE_IMAGES
        CImage cimg;
        ...
        {
            DetectAllEyes(leyes, reyes, image, detpar.eyaw, facerect);
...
#if TRACE_IMAGES
            DrawEyes(cimg, leyes, reyes, ileft_best, iright_best, facerect, detpar.eyaw);
#endif
            if (ileft_best >= 0) // left eye valid?
                RectToImageFrame(detpar.lex, detpar.ley, leyes[ileft_best]);

            if (iright_best >= 0) // right eye valid?
                RectToImageFrame(detpar.rex, detpar.rey, reyes[iright_best]);
        }
        ...
        if (!mouth_det_g.empty()) // need the mouth? (depends on model estart field)
        {
            const Rect mouth_searchrect(
                    MouthSearchRect(facerect, detpar.eyaw,
                                    ileft_best, iright_best, leyes, reyes));
            vec_Rect mouths;
            DetectAllMouths(mouths, image, facerect, mouth_searchrect);
            ...
            if (!mouths.empty()) {
                ...
                if (imouth_best >= 0) // mouth valid?
                {
                    ...
                    RectToImageFrame(detpar.mouthx, detpar.mouthy,
                                     mouths[imouth_best]);
                }
            }
#if TRACE_IMAGES
