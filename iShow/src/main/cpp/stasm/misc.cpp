...
// #if CV_VERSION < 310
// LU() function has refactored to a different place.
#if ((CV_VERSION_MAJOR < 3) || (CV_VERSION_MAJOR == 3) && (CV_VERSION_MINOR < 1)) //&& (CV_VERSION_REVISION == 0)
...
// See also LinSolve() which will solve any linear system.
            ...
            VEC &b)                  // in, but gets destroyed
    {
        ...
        using cv::LU;
#endif
        if (!LU(Buf(mat), mat.cols * sizeof(mat(0)), mat.rows,
 ...
 * @param cascade out
 * @param filename in: basename.ext of cascade
 * @param datadir in
 */
    void OpenDetector(cv::CascadeClassifier &cascade, const char *filename, const char *datadir) {
...
/**
 * @brief convert the x and y coords in feats from the search ROI to the image frame
 * @param feats inout
 * @param searchrect in
 */
    static void DiscountSearchRegion(vec_Rect &feats, const Rect &searchrect) {
        ...
        if (NSIZE(mods) > 1) {
            switch (eyaw) {
                ...
                case EYAW45:
                    ...
                    break;
                default:
                    Err("EyawAsModIndex: bad eyaw %d", eyaw);
                    break;
            }
        }
    ...
    const char *EyawAsString(EYAW eyaw) // utility for debugging/tracing
    {
        switch (int(eyaw)) {
            ...
            case INVALID:
                return "YAW_Inv";
            default:
                Err("YawAsString: Invalid eyaw %d", eyaw);
                break;
        }
    ...
    DetectorParameter FlipDetPar(          // mirror image of detpar
            ...
            int imgwidth) // in
    {
        DetectorParameter detpar_new(detpar);
