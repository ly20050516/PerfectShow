...
void detectFace(const cv::Mat &image, const std::string &image_name/* = std::string()*/) {
    ...
    for (int i = 0; i <= 10; ++i) {
        ...
        cv::Mat gray2;
        cv::warpAffine(gray, gray2, affine, size, cv::INTER_LANCZOS4,
                       cv::BORDER_CONSTANT);  // INTER_CUBIC
...
void mark(const std::string &image_name) {
    ...
    Mat gray = Effect::grayscale(image);
    const std::vector<std::vector<cv::Point2f>> faces = Feature::detectFaces(gray, image_name,
                                                                             CLASSIFIER_DIR);
    if (faces.empty()) {
...
//	cv::line(image, Point2f(line[2], line[3]), Point2f(line[2] + 100*line[0], line[3] + 100*line[1]),  CV_RGB(0, 255, 0), 1, LINE_AA);
    venus::drawLine(image, Point2f(line[2], line[3]),
                    Point2f(line[2] + 100 * line[0], line[3] + 100 * line[1]), CV_RGB(0, 255, 0), 1,
                    LINE_AA);
    ...
    std::cout << "angle2: " "x " << line[0] << " y: " << line[1] << " k " << line[0] / line[1]
              << std::endl;
...
//	assert(image.channels() == 1);  // gray image
    const std::vector<std::vector<Point2f>> faces = Feature::detectFaces(gray, image_name,
                                                                         CLASSIFIER_DIR);
    assert(!faces.empty());
...
void morphology(const cv::Mat &image) {
    ...
    Mat element = getStructuringElement(cv::MORPH_RECT, Size(morph_size, morph_size),
                                        Point(morph_radius, morph_radius));
...
void applyEyeShadow(const std::string &image_name) {
    ...
    stream << PROJECT_DIR << "res/drawable-nodpi/eye_shadow_" << std::setw(3) << std::setfill('0')
           << index << ".png";
...
void applyEyeLash(const std::string &image_name) {
    ...
    int index = 6;  // 0 ~ 9
    std::ostringstream stream;
    stream << PROJECT_DIR << "res/drawable-nodpi/eye_lash_" << std::setw(2) << std::setfill('0')
           << index << ".png";
...
//	cv::imwrite(PROJECT_DIR + "target_brow.png", target_brow);
    ...
    int index = 5;  // 0 ~ 15
    std::ostringstream stream;
    stream << PROJECT_DIR << "res/drawable-nodpi/eye_brow_" << std::setw(2) << std::setfill('0')
           << index << ".png";
    std::string target_brow_filename = stream.str();
...
void applyIris(const std::string &image_name) {
    ...
    for (int i = 0; i < 2; ++i) {
        std::ostringstream stream;
        stream << PROJECT_DIR << "res/drawable-nodpi/iris_" << std::setw(3) << std::setfill('0')
               << index + i << ".png";
        std::string filename = stream.str();
