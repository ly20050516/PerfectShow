...
namespace std {
    ...
    struct hash<cv::Point2f> {
        size_t operator()(const cv::Point2f &point) const noexcept {
            ...
            for (const uint8_t *p = reinterpret_cast<const uint8_t *>(&point), *end =
                    p + sizeof(cv::Point2f); p < end; ++p)
 ...
 */
    void drawLine(cv::Mat &image, const cv::Point2f &pt0, const cv::Point2f &pt1,
...
// draw curve p1~p2 with respect to p0, p3
    ...
    drawCurve(cv::Mat &image, const cv::Point2f &p0, const cv::Point2f &p1, const cv::Point2f &p2,
              const cv::Point2f &p3, const cv::Scalar &color,
              int thickness = 1, int lineType = cv::LINE_8, int shift = 0);

    void drawCrossHair(cv::Mat &image, const cv::Point2f &position, int radius = 20,
                       const cv::Scalar &color = cv::Scalar(0, 255, 0, 255), int gap = 4,
                       int thickness = 1, int lineType = cv::LINE_8, int shift = 0);
