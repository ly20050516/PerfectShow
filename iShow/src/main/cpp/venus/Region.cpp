...
//		uint8_t value = mask.at<uint8_t>(r, c);
        ...
        */
        ...
        for (int i = mask.cols - 1; i >= left; --i)
            if (row[i] != 0) {
                right = i;
                break;
            }
    ...
    T colorDifference(const T *color, const T *reference_color, int channel, bool has_alpha,
                      T threshold, SelectCriterion criterion, bool antialias,
        ...
        static_assert(std::is_floating_point<T>::value ||
                      ...
                      "limit to integral/floating primitive type");
        assert(criterion == SelectCriterion::COMPOSITE ||
               channel >= 3);  // RGB or HSV mode have 3 channels.
        constexpr T ZERO(0), FULL(
...
//		return clamp(x, ZERO, FULL/2) * 2;
    ...
    void Region::selectContiguousRegionByColor(cv::Mat &mask, const cv::Mat &image,
                                               const cv::Vec4b &color, uint8_t threshold,
                                               SelectCriterion criterion, bool select_transparent,
                                               bool antialias) {
...
#pragma omp parallel for
        for (int i = 0; i < length; ++i)
            mask_color[i] = colorDifference(_image_color + i * nb_channel, reference_color,
                                            nb_channel, has_alpha,
                                            threshold, criterion, antialias, select_transparent);
    }

    void Region::selectContiguousRegionByColor(cv::Mat &mask, const cv::Mat &image,
                                               const cv::Vec4f &color, float threshold,
                                               SelectCriterion criterion, bool select_transparent,
                                               bool antialias) {
...
#pragma omp parallel for
    ...
    void Region::shrink(cv::Mat &dst, const cv::Mat &src, int offset) {
        ...
        int size = radius * 2 + 1;
        Mat kernel = cv::getStructuringElement(cv::MORPH_ELLIPSE, Size(size, size),
                                               Point(radius, radius));

        // Apply the specified morphology operation
        const Point anchor(-1,
                           -1);  // default value (-1, -1) means that the anchor is at the element center.
        int iterations = 1;
        if (offset > 0)
    ...
    void Region::overlay(cv::Mat &dst, const cv::Mat &patch, const cv::Point2i &position,
                         const cv::Mat &mask) {
    ...
    cv::Mat Region::transform(cv::Size &size, cv::Point2f &pivot, const float &angle,
...
*/
        ...
        std::vector<Point2f> points{pivot, Point2f(0, 0), Point2f(x, 0), Point2f(0, y),
                                    Point2f(x, y)};
    ...
    void
    Region::snake(const cv::Mat1f &image, std::vector<cv::Point2f> &points, float alpha, float beta,
                  float gamma, float kappa, const cv::Vec3f &weight, int nb_iteration) {
        ...
        for (int r = 1, r_end = image.rows - 1; r < r_end; ++r)
            for (int c = 1, c_end = image.cols - 1; c < c_end; ++c) {
                ...
                const float &_00 = image.at<float>(r - 1, c - 1), &_01 = image.at<float>(r - 1,
                                                                                         c), &_02 = image.at<float>(
                        r - 1, c + 1);
                const float &_10 = image.at<float>(r, c - 1), &_11 = image.at<float>(r,
                                                                                     c), &_12 = image.at<float>(
                        r, c + 1);
                const float &_20 = image.at<float>(r + 1, c - 1), &_21 = image.at<float>(r + 1,
                                                                                         c), &_22 = image.at<float>(
                        r + 1, c + 1);

                float gradient_x = _12 - _10;
                float gradient_y = _21 - _01;
                e_edge.at<float>(r, c) = std::sqrt(
...
//		float dxy= (_11 - _12) + (_22 - _21);  // [1 -1; -1 1]
                ...
                float dx_dx = dx * dx;
                float dy_dy = dy * dy;
                e_term.at<float>(r, c) = (dx_dx * dyy - 2 * dxy * dx * dy + dy_dy * dxx) /
                                         std::pow(1 + dx_dx + dy_dy, 1.5F);
            }
