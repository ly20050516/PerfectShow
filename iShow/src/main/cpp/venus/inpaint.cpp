...
namespace venus {
    ...
    void TemplateMatchCandidates::findCandidates(const cv::Mat &templ, const cv::Mat &templMask,
                                                 cv::Mat &candidates,
                                                 int maxWeakErrors, float maxMeanDifference) {
        CV_Assert(templ.type() == CV_MAKETYPE(CV_8U, static_cast<int>(_integrals.size())) &&
        ...
        for (size_t i = 0; i < _integrals.size(); ++i) {
            ...
            for (int ty = 0; ty < candidates.rows; ++ty) {
                ...
                for (int tx = 0; tx < candidates.cols; ++tx) {
                    ...
                    outputRow[tx] = compareWeakClassifiers(integral, tx, ty, templ.size(), blocks,
                                                           referenceClassRow,
                                                           (float) templMean[static_cast<int>(i)],
                                                           maxMeanDifference, maxWeakErrors);
                }
            }
        }
    }

    void TemplateMatchCandidates::weakClassifiersForTemplate(const cv::Mat &templ,
                                                             const cv::Mat &templMask,
                                                             const std::vector<cv::Rect> &rects,
                                                             cv::Mat1i &classifiers,
                                                             cv::Scalar &mean) {
        ...
        for (int x = 0; x < size; ++x) {
            cv::Scalar blockMean = cv::mean(templ(rects[x]),
                                            templMask.empty() ? cv::noArray() : templMask(
                                                    rects[x]));
    ...
    unsigned char TemplateMatchCandidates::compareWeakClassifiers(const cv::Mat1i &i, int x, int y,
        ...
        const float posMean =
                (bottomRow[x + templSize.width] - bottomRow[x] - topRow[x + templSize.width] +
                 topRow[x]) / static_cast<float>(templSize.area());
        ...
        for (size_t r = 0; r < blocks.size(); ++r) {
            ...
            const float blockMean =
                    (bottomRow[ox + b.width] - bottomRow[ox] - topRow[ox + b.width] + topRow[ox]) /
                    static_cast<float>(b.width * b.height);
    ...
    void TemplateMatchCandidates::computeBlockRects(cv::Size size, cv::Size partitions,
    ...
    void TemplateMatchCandidates::removeInvalidBlocks(const cv::Mat &templMask,
                                                      std::vector<cv::Rect> &rects) {
        if (templMask.empty())
            return;
...
/** Flags for creating patch. */
    enum PatchFlags {
        ...
        PATCH_REF =
        1 << 1,  ///< Reference parent memory. Slower, but keeps the parent memory alive.
    };
 ...
 */
    inline std::pair<cv::Rect, cv::Rect>
    comparablePatchRegions(const cv::Mat &a, const cv::Mat &b, cv::Point ap, cv::Point bp,
                           int halfPatchSize) {
...
/** Test if patch goes across the boundary. */
    inline bool
    isCenteredPatchCrossingBoundary(cv::Point p, int halfPatchSize, const cv::Mat &img) {
    ...
    void Inpainter::initialize() {
        CV_Assert(_input.image.channels() == 3 && _input.image.depth() == CV_8U &&
                  ...
                  (_input.sourceMask.empty() ||
                   _input.targetMask.size() == _input.sourceMask.size()) &&
                  _input.patchSize > 0);
        ...
        // Initialize regions
        cv::rectangle(_targetRegion, cv::Rect(0, 0, _targetRegion.cols, _targetRegion.rows),
                      cv::Scalar(0), _halfMatchSize);

        _sourceRegion = 255 - _targetRegion;
        cv::rectangle(_sourceRegion, cv::Rect(0, 0, _sourceRegion.cols, _sourceRegion.rows),
                      cv::Scalar(0), _halfMatchSize);
        cv::erode(_sourceRegion, _sourceRegion, cv::getStructuringElement(cv::MORPH_RECT, cv::Size(
                _halfMatchSize * 2 + 1, _halfMatchSize * 2 + 1)));
    ...
    cv::Point Inpainter::findSourcePatchLocation(const cv::Point &targetPatchLocation,
                                                 bool useCandidateFilter) {
        cv::Point bestLocation(-1, -1);
        float bestError = std::numeric_limits<float>::max();

        cv::Mat3b targetImagePatch = centeredPatch<PATCHFLAGS>(_image, targetPatchLocation.y,
                                                               targetPatchLocation.x,
                                                               _halfMatchSize);
        cv::Mat1b targetMask = centeredPatch<PATCHFLAGS>(_targetRegion, targetPatchLocation.y,
                                                         targetPatchLocation.x, _halfMatchSize);
        ...
        for (int y = _startY; y < _endY; ++y)
            for (int x = _startX; x < _endX; ++x) {
                ...
                const bool shouldTest = (!useCandidateFilter ||
                                         _candidates.at<uchar>(y - _halfMatchSize,
                                                               x - _halfMatchSize)) &&
                                        _sourceRegion.at<uchar>(y, x) > 0;

                if (shouldTest) {
                    ++count;
                    cv::Mat1b sourceMask = centeredPatch<PATCHFLAGS>(_sourceRegion, y, x,
                                                                     _halfMatchSize);
                    cv::Mat3b sourceImagePatch = centeredPatch<PATCHFLAGS>(_image, y, x,
                                                                           _halfMatchSize);

                    float error = (float) cv::norm(targetImagePatch, sourceImagePatch, cv::NORM_L1,
                                                   invTargetMask);
    ...
    void Inpainter::propagatePatch(const cv::Point &target, const cv::Point &source) {
        cv::Mat1b copyMask = centeredPatch<PATCHFLAGS>(_targetRegion, target.y, target.x,
                                                       _halfPatchSize);
        ...
        centeredPatch<PATCHFLAGS>(_confidence, target.y, target.x, _halfPatchSize).setTo(cPatch,
                                                                                         copyMask);
