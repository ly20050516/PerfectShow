...
*/
    cv::Point2f
    centerOfCircumscribedCircle(const cv::Point2f &A, const cv::Point2f &B, const cv::Point2f &C) {
...
#pragma omp parallel for
    ...
    void drawLine(Mat &image, const Point2f &pt0, const Point2f &pt1, const Scalar &color,
        ...
        Point2f p0(0, 0), p1(static_cast<float>(image.cols - 1),
    ...
    void
    drawRotatedRect(cv::Mat &image, const cv::RotatedRect &rotated_rect, const cv::Scalar &color,
                    int thickness/* = 1 */, int lineType/* = cv::LINE_8 */, int shift/* = 0 */) {
        constexpr int N = 4;
        Point2f vertices[N];
    ...
    void
    drawCurve(cv::Mat &image, const cv::Point2f &p0, const cv::Point2f &p1, const cv::Point2f &p2,
              const cv::Point2f &p3,
    ...
    void drawCrossHair(cv::Mat &image, const cv::Point2f &position, int radius/* = 20 */,
                       const cv::Scalar &color/* = cv::Scalar(0, 255, 0, 255) */, int gap/* = 2 */,
                       int thickness/* = 1 */, int lineType/* = cv::LINE_8 */, int shift/* = 0 */) {
    ...
    void drawCross(cv::Mat &image, const cv::Point2f &position, int radius, const Scalar &color,
                   int thickness/* = 1 */, int lineType/* = cv::LINE_8 */, int shift/* = 0 */) {
        cv::Point2f p00(position.x - radius, position.y - radius), p01(position.x - radius,
                                                                       position.y + radius);
        cv::Point2f p10(position.x + radius, position.y - radius), p11(position.x + radius,
                                                                       position.y + radius);
...
*/
    cv::Point2f
    catmullRomSpline(float t, const cv::Point2f &p0, const cv::Point2f &p1, const cv::Point2f &p2,
                     const cv::Point2f &p3) {
//	assert(0 <= t && t <= 1);
    ...
    cv::Point2f
    bezier2(float t, const cv::Point2f &p0, const cv::Point2f &p1, const cv::Point2f &p2) {
        float lmt = 1 - t;
//	return lmt * bezier1(t, p0, p1) + t * bezier1(t, p1, p2);
    ...
    cv::Point2f
    bezier3(float t, const cv::Point2f &p0, const cv::Point2f &p1, const cv::Point2f &p2,
            const cv::Point2f &p3) {
                ...
                for (int dy = -radius; dy < radius; ++dy) {
                    ...
                    for (int dx = -radius; dx < radius; ++dx) {
                        ...
                        if (0 <= x && x < cols && 0 <= y && y < rows)
                            if (dx * dx + dy * dy <=
                                rr)  // comment this line if you want to detect squared region.
                            {
                                ++sum;
    ...
    void gradient(const cv::Mat &image, cv::Mat *dx, cv::Mat *dy/* = nullptr */) {
        ...
        for (int r = 0; r < rows; ++r)
            for (int c = 0; c < cols; ++c) {
                ...
                switch (c2 - c0) {
                    case 2:
                        dx->at<float>(r, c) = diff / 2;
                        break;
                    case 1:
                        dx->at<float>(r, c) = diff;
                        break;
                    case 0:
                        dx->at<float>(r, c) = 0;
                        break;
        ...
        for (int r = 0; r < rows; ++r) {
            ...
            for (int c = 0; c < cols; ++c) {
                ...
                switch (r2 - r0) {
                    case 2:
                        dy->at<float>(r, c) = diff / 2;
                        break;
                    case 1:
                        dy->at<float>(r, c) = diff;
                        break;
                    case 0:
                        dy->at<float>(r, c) = 0;
                        break;
