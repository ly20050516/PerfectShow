...
// used after subdiv.getTriangleList(triangles); filter those triangles that lies on border.
    ...
    static void
    drawDelaunay(cv::Mat &image, const std::vector<cv::Vec6f> &triangles, const cv::Scalar &color) {
    ...
    void Feature::triangulate(cv::Mat &image, const std::vector<cv::Point2f> &points,
                              const std::vector<cv::Vec3b> &triangles) {
        const int width = image.cols, height = image.rows;
        const cv::Scalar line_color = CV_RGB(255, 0, 255), text_color = CV_RGB(0, 255, 0);

        const float font_scale = (std::min(width, height) <= 720) ? std::min(width, height) / 720.0f
                                                                  : 1.0f;
...
//		points.push_back(O + vOB.rotate(+theta/3));
    ...
    std::vector<cv::Point2f> Feature::detectFace(const cv::Mat &image, const std::string &tag,
                                                 const std::string &classifier_dir) {
    ...
    Feature::detectFaces(const cv::Mat &image, const std::string &tag,
                         const std::string &classifier_dir) {
        ...
        if (!stasm_open_image(image_data, image.cols, image.rows, image_path, allow_multiple_faces,
...
//		correctIris(image, points);
    ...
    std::vector<std::vector<cv::Point2f>>
    Feature::detectFaces(cv::Size2i *size, const std::string &image_name,
                         const std::string &classifier_dir) {
...
//	Region::inset(rect, 1);
        ...
        for (size_t i = 0; i < points.size(); ++i) {
            ...
            cv::putText(image, venus::to_string(i), Point2i(pt.x + radius, pt.y), font_face,
                        font_scale, text_color, 1, LINE_AA);
//		image.at<Vec3b>(cvRound(point.y), cvRound(point.x)) = Vec3b(0, 255, 0);
    ...
    void Feature::markWithIndices(cv::Mat &image, const std::vector<cv::Point2f> &points) {
        ...
        for (size_t i = 0; i < point_count; ++i) {
            const Point2f &point = points[i];
            Point2i pt(cvRound(point.x), cvRound(point.y));
            cv::putText(image, to_string(i), Point2i(pt.x + offset, pt.y), font_face, font_scale,
                        COLOR0, 1, LINE_AA);
...
//	cv::circle(image, eye_brow_top_l, 1, COLOR2, 1, LINE_AA);
        ...
        auto drawHorizontalLine = [](cv::Mat &image, const Point2f &point, float k,
                                     const Scalar &color) {
            Point2f right(0, 0), left(static_cast<float>(image.cols - 1),
...
*/
        ...
        float t = (line[0] * (center.x - line[2]) + line[1] * (center.y - line[3])) /
...
//	float theta = std::atan2(x, y);
    ...
    cv::Mat
    Feature::createMask(const std::vector<cv::Point2f> &points, float blur_radius/* = 0.0F */,
                        cv::Point2i *position/* = nullptr */) {
...
//	Rect rect = cv::boundingRect(points);
    ...
    cv::Mat
    Feature::maskPolygon(const cv::Rect2i &rect, const std::vector<cv::Point2f> &points, int start,
                         int length) {
    ...
    cv::Mat
    Feature::maskPolygonSmooth(const cv::Rect2i &rect, const std::vector<cv::Point2f> &points,
                               const int level/* = 8 */) {
        assert(level > 0);
...
//	Rect rect = cv::boundingRect(points);  // expose rect as parameter
        ...
        for (int j = 1; j <= level; ++j) {
            float FACTOR = std::sqrt(
                    1.0F - static_cast<float>(j - 1) / level);  // every step shrinks a little bit.
    ...
    Feature::calculateBrowPolygon(const std::vector<cv::Point2f> &points, bool right) {
        assert(points.size() == COUNT);
/*
...
//	polygon.push_back(catmullRomSpline(0.50F, _22, _23, _24, _25));
    ...
    Region
    Feature::calculateBrowRegion(const std::vector<cv::Point2f> &points, const cv::Vec4f &line,
                                 bool right) {
...
//	Rect2i rect = cv::boundingRect(polygon);
    ...
    std::vector<cv::Point2f>
    Feature::calculateEyePolygon(const std::vector<cv::Point2f> &points, bool right) {
/*
...
*/
    ...
    Feature::calculateIrisInfo(const std::vector<cv::Point2f> &points, bool right) {
        assert(points.size() == COUNT);
...
//	float radius = *std::min_element(d, d + 4);
    ...
    cv::RotatedRect
    Feature::calculateBlushRectangle(const std::vector<cv::Point2f> &points, bool right) {
        Vec4f line = Feature::getSymmetryAxis(points);
    ...
    cv::RotatedRect
    Feature::calculateBlushRectangle(const std::vector<cv::Point2f> &points, float angle,
                                     bool right) {
        // symmetric point
    ...
    std::vector<cv::Point2f>
    Feature::calculateBlushPolygon(const std::vector<cv::Point2f> &points, bool right) {
        assert(points.size() == COUNT);
    ...
    std::vector<cv::Point2f>
    Feature::calculateLipPolygon(const std::vector<cv::Point2f> &points, bool upper) {
        assert(points.size() == COUNT);
/*
...
*/
        ...
        } else  // for lower lip
        {
            ...
            for (int i = 76; i <= 79; ++i) {
                ...
                int i3 = i == 79 ? 63 : i + 2;
                polygon.push_back(
                        catmullRomSpline(1.0F / 3, points[i0], points[i1], points[i2], points[i3]));
                polygon.push_back(
                        catmullRomSpline(2.0F / 3, points[i0], points[i1], points[i2], points[i3]));
    ...
    std::vector<cv::Point2f>
    Feature::calculateTeethPolygon(const std::vector<cv::Point2f> &points) {
        assert(points.size() == COUNT);
        return std::vector<Point2f>
                {
    ...
    Region
    Feature::calculateEyeRegion(const std::vector<cv::Point2f> &points, const cv::Vec4f &line,
                                bool right) {
    ...
    Region
    Feature::calculateLipsRegion(const std::vector<cv::Point2f> &points, const cv::Vec4f &line) {
...
//	Mat mask;
    ...
    cv::RotatedRect
    Feature::calculateRectangle(float angle, const cv::Point2f &left, const cv::Point2f &top,
                                const cv::Point2f &right, const cv::Point2f &bottom) {
    ...
    cv::Vec4f
    Feature::calcuateDistance(cv::Point2f &pivot, const cv::Point2f &left, const cv::Point2f &top,
                              const cv::Point2f &right, const cv::Point2f &bottom) {
...
#else
        float denorm =
                (left.x - right.x) * (top.y - bottom.y) - (left.y - right.y) * (top.x - bottom.x);
    ...
    cv::Vec4f
    Feature::calcuateEyeRadius(const std::vector<cv::Point2f> &points, const cv::Vec4f &line,
                               bool right) {
    ...
    cv::Mat Feature::maskSkinRegion(int width, int height, const std::vector<cv::Point2f> &points) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            polygon[3 * i + 0] = catmullRomSpline(0.25F, points[_0], points[_1], points[_2],
                                                  points[_3]);
            polygon[3 * i + 1] = catmullRomSpline(0.50F, points[_0], points[_1], points[_2],
                                                  points[_3]);
            polygon[3 * i + 2] = catmullRomSpline(0.75F, points[_0], points[_1], points[_2],
                                                  points[_3]);
        }
