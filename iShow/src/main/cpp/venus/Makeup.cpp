...
#pragma omp parallel for
    ...
    Makeup::createHeartPolygon(const cv::Point2f &center, float radius, float angle/* = 0.0F*/) {
    ...
    std::vector<cv::Point2f>
    Makeup::createPolygon(const std::vector<cv::Point2f> &points, BlushShape shape, bool right) {
...
//		Point2f nose_t = (points[34] + points[44] + points[53])/3;
    ...
    void Makeup::blend(cv::Mat &result, const cv::Mat &dst, const cv::Mat &src,
    ...
    void Makeup::blend(cv::Mat &result, const cv::Mat &dst, const cv::Mat &src, const cv::Mat &mask,
                       const cv::Point2i &origin, float amount) {
        assert(!src.empty() && (src.type() == CV_8UC4 || src.type() == CV_32FC4));
        assert(mask.type() == CV_8UC1);
        if (dst.data != result.data)
    ...
    void Makeup::applyBrow(cv::Mat &dst, const cv::Mat &src, const std::vector<cv::Point2f> &points,
                           const cv::Mat &brow, uint32_t color, float amount,
                           float offsetY/* = 0.0F */) {
...
//	std::cout << __FUNCTION__ << " angle: " << rad2deg(angle) << '\n';
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            Mat target_mask(rect_with_margin.height, rect_with_margin.width, CV_8UC1,
                            Scalar::all(0));
...
#pragma omp parallel for
            for (int c = 0; c < target_mask.cols; ++c) {
                ...
                for (int r = r0; r < r1; ++r) {
                    ...
                    weight = smoothStep(0.42F, 0.78F,
                                        weight);  // prefer to use skin of eyes above than below.
                    roi.at<Vec3b>(r, c) = mix(roi.at<Vec3b>(r0_outter, c),
                                              roi.at<Vec3b>(r1_outter, c), weight);
                }
            }
...
//		cv::imshow("target_mask", target_mask);
    ...
    //			*reinterpret_cast<cv::Vec3b*>(&dst_color) = src_color;
            ...
            cv::Mat affined_mask;
            cv::warpAffine(makeup_mask, affined_mask, affine, target_size, cv::INTER_LINEAR,
                           cv::BORDER_CONSTANT);
    ...
    void Makeup::applyEye(cv::Mat &dst, const cv::Mat &src, const std::vector<cv::Point2f> &points,
...
//		cv::imshow("test" + std::to_string(j), _cosmetic);
    ...
    void
    Makeup::applyEyeLash(cv::Mat &dst, const cv::Mat &src, const std::vector<cv::Point2f> &points,
    ...
    cv::Mat Makeup::createEyeShadow(cv::Mat mask[3], uint32_t color[3]/*, const int& COUNT = 3 */) {
        ...
        auto unpack = [](uint32_t c) -> cv::Vec3i { return cv::Vec3i(c & 0xFF, (c >> 8) & 0xFF,
                                                                     (c >> 16) &
                                                                     0xFF/*, (c>>24) & 0xFF*/);
        };
    ...
    void
    Makeup::applyEyeShadow(cv::Mat &dst, const cv::Mat &src, const std::vector<cv::Point2f> &points,
                           cv::Mat mask[3], uint32_t color[3], float amount) {
        Mat eye_shadow = createEyeShadow(mask, color);
        applyEye(dst, src, points, eye_shadow, amount);
    }

    void Makeup::applyIris(cv::Mat &dst, const cv::Mat &src, const std::vector<cv::Point2f> &points,
...
//	cv::imwrite("color2alpha.png", mask2);
        ...
        for (int i = 0; i < 2; ++i) {
            const bool is_right = i == 0;
            const std::pair<cv::Point2f, float> iris_info = Feature::calculateIrisInfo(points,
                                                                                       is_right);
    ...
    void
    Makeup::applyBlush(cv::Mat &dst, const cv::Mat &src, const std::vector<cv::Point2f> &points,
                       BlushShape shape, uint32_t color, float amount) {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            Mat mask = Feature::maskPolygonSmooth(rect, polygon,
                                                  8);  // level (here 8) can be tuned.
...
//		cv::imshow(std::string("blush mask ") + (i == 0 ? "right":"left"), mask);
    ...
    Makeup::applyBlush(cv::Mat &dst, const cv::Mat &src, const std::vector<cv::Point2f> &points,
                       const cv::Mat &mask, uint32_t color, float amount) {
        assert(!src.empty() && points.size() == Feature::COUNT);
        assert(!mask.empty() && mask.type() ==
                                CV_8UC1);  // In fact, relaxing CV_8UC1 restriction can be achieved by Effect::grayscale()
        assert(0.0F <= amount && amount <= 1.0F);
...
//	mask2 = Effect::grayscale(mask2);  // relaxation can be done here.
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            if (is_square_shape)
                ...
                Point2f middle_top = (points[40] + points[50]) -
                                     (points[42] + points[43]) / 2;  // (points[40] + points[50])/2;
                Point2f middle_bottom = (points[53] + points[56] * 2) /
                                        3;  // points[56]; (points[53] + points[56])/2;

                rotated_rect = is_right ?
                               Feature::calculateRectangle(angle, points[2], middle_top,
                                                           middle_bottom, middle_bottom) :
                               Feature::calculateRectangle(angle, middle_bottom, middle_top,
                                                           points[10], middle_bottom);
            }
...
//		venus::rectangle(dst, rotated_rect, Scalar(0, 255, 0));
            ...
            Mat affined_mask;
            cv::warpAffine(mask2, affined_mask, affine, size, cv::INTER_LINEAR,
                           cv::BORDER_CONSTANT);
    ...
    void Makeup::applyLip(cv::Mat &dst, const cv::Mat &src, const std::vector<cv::Point2f> &points,
                          uint32_t color, float amount) {
