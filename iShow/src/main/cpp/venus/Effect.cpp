...
#pragma omp parallel for
    ...
    void Effect::mapColor(cv::Mat &dst, const cv::Mat &src, const uint8_t *mask,
                ...
                for (int r = 0; r < src.rows; ++r)
                    for (int c = 0; c < src.cols; ++c) {
                        ...
                        for (int i = 0; i < 3; ++i)  // loop unrolling
                            dst_color[i] = static_cast<uint8_t>(src_color[i] * l_amount +
                                                                target[i]);
                    }
                break;
            case CV_32FC4:
...
#pragma omp parallel for
    ...
    void Effect::colorize(cv::Mat &dst, const cv::Mat &src, float hue/* = 0.0F */,
                          float saturation/* = 0.5F */, float lightness/* = 0.0F*/) {
...
#endif
        for (int i = 0; i < length; i += 4) {
            float luminance = LUMINANCE_R * src_color[_0] + LUMINANCE_G * src_color[_1] +
                              LUMINANCE_B * src_color[_2];
...
//		dst_color[3]  = src_color[3];  // already copied
    ...
    void Effect::unsharpMask(cv::Mat &dst, const cv::Mat &src, float radius/* = 5.0F*/,
                             int threshold/* = 0*/, float amount/* = 0.5F*/) {
        assert(1.0F <= radius);
    ...
    float Effect::mapColorBalance(float value, float lightness, float shadows, float midtones,
                                  float highlights) {
...
//	constexpr // g++ pass, but clang fails with error: constexpr variable 'clamp_01' must be initialized by a constant expression.
    ...
    void Effect::adjustColorBalance(cv::Mat &dst, const cv::Mat &src, const cv::Vec3f config[3],
        ...
        for (int i = 0; i < length; i += channels) {
            ...
            for (int k = 0; k < 3; ++k)
                dst_rgb[k] = mapColorBalance(src_rgb[k], lightness, config[RANGE_SHADOW][k],
                                             config[RANGE_MIDTONE][k], config[RANGE_HIGHLIGHT][k]);
    ...
    void Effect::adjustBrightnessAndContrast(Mat &dst, const Mat &src, float brightness/* = 0.0F */,
                                             float contrast/* = 1.0F */) {
...
#pragma omp parallel for
                for (int i = 0; i < length; i += 4) {
                    data[i + 0] = clamp(
                            (data[i + 0] - threshold) * contrast + brightness + threshold);
                    data[i + 1] = clamp(
                            (data[i + 1] - threshold) * contrast + brightness + threshold);
                    data[i + 2] = clamp(
                            (data[i + 2] - threshold) * contrast + brightness + threshold);
                }
            } else
#pragma omp parallel for
                for (int i = 0; i < length; ++i)
...
#pragma omp parallel for
    ...
    void Effect::adjustHueSaturation(cv::Mat &dst, const cv::Mat &src, float hue/* = 0.0F */,
                                     float saturation/* = 1.0F */, float lightness/* = 0.0F */) {
