...
namespace venus {
    ...
    cv::Mat ImageWarp::setAllAndGenerate(const cv::Mat &src,
                                         ...
                                         const cv::Size &target, float alpha,
                                         float amount/* = 1.0F */) {
    ...
    cv::Mat ImageWarp::genNewImage(const cv::Mat &src, float amount) {
        ...
        for (int i = 0; i < dst_size.height; i += grid_size)
            for (int j = 0; j < dst_size.width; j += grid_size) {
                ...
                if (ni >= dst_size.height)
                    ni = dst_size.height - 1, h = static_cast<float>(ni - i + 1);
                if (nj >= dst_size.width)
                    nj = dst_size.width - 1, w = static_cast<float>(nj - j + 1);

                for (int di = 0; di < h; ++di)
                    for (int dj = 0; dj < w; ++dj) {
                        float deltaX = bilinear_interp(di / h, dj / w, rDx(i, j), rDx(i, nj),
                                                       rDx(ni, j), rDx(ni, nj));
                        float deltaY = bilinear_interp(di / h, dj / w, rDy(i, j), rDy(i, nj),
                                                       rDy(ni, j), rDy(ni, nj));
                        ...
                        switch (src.channels()) {
                            ...
                            case 3:
                                for (int b = 0; b < 3; ++b)
                                    dst.at<Vec3b>(i + di, j + dj)[b] = bilinear_interp(ny - y0,
                                                                                       nx - x0,
                                                                                       src.at<Vec3b>(
                                                                                               y0,
                                                                                               x0)[b],
                            ...
                            case 4: {
                                for (int b = 0; b < 4; ++b)
                                    dst.at<Vec4b>(i + di, j + dj)[b] = bilinear_interp(ny - y0,
                                                                                       nx - x0,
                                                                                       src.at<Vec4b>(
                                                                                               y0,
                                                                                               x0)[b],
    ...
    void ImageWarp::setMappingPoints(const std::vector<cv::Point2f> &dst_points,
...
//				float denorm = distance(Point2f(i, j), src_points[k]);
                ...
                if (k == N) {
                    ...
                    for (k = 0; k < N; ++k) {
                        ...
                        tmpP.y =
                                -Pi.dot(curVJ) * dst_points[k].x + PiJ.dot(curVJ) * dst_points[k].y;
                        tmpP *= w[k] / miu_r;
                        newP += tmpP;
                    }
                    newP += qstar;
