...
// blend mode for uint8_t version
    ...
    inline uint8_t blendScreen(uint8_t A, uint8_t B) {
        return 255 - ((255 - A) * (255 - B) + 127) / 255;
    }

    inline uint8_t blendExclusion(uint8_t A, uint8_t B) { return A + B - (2 * A * B + 127) / 255; }

    inline uint8_t blendOverlay(uint8_t A, uint8_t B) {
        return (uint8_t) ((B < 128 ? 2 * A * B : (255 - 2 * (255 - A) * (255 - B)) + 127) / 255);
    }

    inline uint8_t blendSoftLight(uint8_t A, uint8_t B) {
        return (B < 128) ? ((2 * ((A >> 1) + 64)) * B + 127) / 255 : (255 - (2 * (255 - ((A >> 1) +
                                                                                         64)) *
                                                                             (255 - B) + 127) /
                                                                            255);
    }
    ...
    inline uint8_t blendColorDodge(uint8_t A, uint8_t B) {
        return (B == 255) ? 255 : std::min(255, A * 255 / (255 - B));
    }

    inline uint8_t blendColorBurn(uint8_t A, uint8_t B) {
    ...
    inline uint8_t blendLinearLight(uint8_t A, uint8_t B) {
        return (B < 128) ? blendLinearBurn(A, 2 * B) : blendLinearDodge(A, 2 * (B - 128));
    }

    inline uint8_t blendVividLight(uint8_t A, uint8_t B) {
        return (B < 128) ? blendColorBurn(A, 2 * B) : blendColorDodge(A, 2 * (B - 128));
    }

    inline uint8_t blendPinLight(uint8_t A, uint8_t B) {
        return (B < 128) ? blendDarken(A, 2 * B) : blendLighten(A, 2 * (B - 128));
    }

    inline uint8_t blendHardMix(uint8_t A, uint8_t B) {
        return blendVividLight(A, B) < 128 ? 0 : 255;
    }

    inline uint8_t blendReflect(uint8_t A, uint8_t B) {
        return (B == 255) ? 255 : std::min(255, A * A / (255 - B));
    }

    inline uint8_t blendGlow(uint8_t A, uint8_t B) { return blendReflect(B, A); }

    inline uint8_t blendPhoenix(uint8_t A, uint8_t B) {
        return std::min(A, B) - std::max(A, B) + 255;
    }

    inline uint8_t blendAlpha(uint8_t A, uint8_t B, uint8_t O) {
        return lerp(B, A, O);/*O * A + (1 - O) * B*/ }

    inline uint8_t blendAlphaF(uint8_t A, uint8_t B, uint8_t (*F)(uint8_t, uint8_t),
                               uint8_t O) { return blendAlpha(F(A, B), A, O); }
...
// blend mode for float version
    ...
    inline float blendOverlay(float A, float B) {
        return (B <= 0.5F) ? (2 * A * B) : (1.0F - 2 * (1.0F - A) * (1.0F - B));
    }

    inline float blendSoftLight(float A, float B) {
        return (B <= 0.5F) ? (A + 0.5F) * B : (1.0F - (1.5F - A) * (1.0F - B));
    }

    inline float blendHardLight(float A, float B) { return blendOverlay(B, A); }

    inline float blendColorDodge(float A, float B) {
        return fuzzyEqual(B, 1.0F) ? B : std::min(1.0F, A * 1.0F / (1.0F - B));
    }

    inline float blendColorBurn(float A, float B) {
        return fuzzyEqual(B, 0.0F) ? B : std::max(0.0F, 1.0F - (1.0F - A) * 1.0F / B);
    }
    ...
    inline float blendLinearLight(float A, float B) {
        return (B <= 0.5F) ? blendLinearBurn(A, 2 * B) : blendLinearDodge(A, 2 * (B - 0.5F));
    }

    inline float blendVividLight(float A, float B) {
        return (B <= 0.5F) ? blendColorBurn(A, 2 * B) : blendColorDodge(A, 2 * (B - 0.5F));
    }

    inline float blendPinLight(float A, float B) {
        return (B <= 0.5F) ? blendDarken(A, 2 * B) : blendLighten(A, 2 * (B - 0.5F));
    }

    inline float blendHardMix(float A, float B) { return blendVividLight(A, B) <= 0.5F ? 0 : 1.0F; }

    inline float blendReflect(float A, float B) {
        return fuzzyEqual(B, 1.0F) ? B : std::min(1.0F, A * A / (1.0F - B));
    }
    ...
    inline float blendAlpha(float A, float B, float O) {
        return lerp(B, A, O);/*O * A + (1 - O) * B*/ }

    inline float blendAlphaF(float A, float B, float (*F)(float, float), float O) {
