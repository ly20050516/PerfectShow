...
*/
    ...
    private:
        ...
        void weakClassifiersForTemplate(const cv::Mat &templ, const cv::Mat &templMask,
                                        const std::vector<cv::Rect> &rects, cv::Mat1i &classifiers,
                                        cv::Scalar &mean);

        /** Compare the template classifiers to the classifiers generated from the given template position. */
        unsigned char
        compareWeakClassifiers(const cv::Mat1i &i, int x, int y, const cv::Size &templSize,
                               const std::vector<cv::Rect> &blocks, const int *compareTo,
                               float templateMean, float maxMeanDiff, int maxWeakErrors);
    ...
    public:
         ...
         */
        void findCandidates(const cv::Mat &templ, const cv::Mat &templMask, cv::Mat &candidates,
                            int maxWeakErrors = 3, float maxMeanDifference = 20);
    };
...
*/
    ...
    private:
        ...
        cv::Point
        findSourcePatchLocation(const cv::Point &targetPatchLocation, bool useCandidateFilter);
