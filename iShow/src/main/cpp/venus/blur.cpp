...
namespace venus {
    ...
    void gaussianBlur(cv::Mat &dst, const cv::Mat &src, float radius) {
        ...
        int width = (r << 1) + 1;
        double std_dev =
                radius * 3;  // 3-sigma rule https://en.wikipedia.org/wiki/68�C95�C99.7_rule

        cv::GaussianBlur(src, dst, Size(width, width), std_dev, std_dev,
...
// Radius can be tuned, but dont't make the radius too large, since it's time-consuming.
    ...
    static cv::Vec4b
    getBlurredValue(const cv::Mat &image, const cv::Point2i &point, const float &blur_radius) {
        ...
        for (int r = 0; r < src.rows; ++r)
            for (int c = 0; c < src.cols; ++c) {
                ...
                if (distance >= outer_radius)
                    ...
                    dst.at<Vec4b>(r, c) = getBlurredValue(src, point,
                                                          distance / outer_radius * BLUR_RADIUS);
                else  // distance < inner_radius
                    dst.at<Vec4b>(r, c) = src.at<Vec4b>(r, c);
#endif
            }
