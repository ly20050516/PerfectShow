 ...
 */
    ...
    private:
        ...
        static void triangulate(cv::Mat &image, const std::vector<cv::Point2f> &points,
                                const std::vector<cv::Vec3b> &triangles);

        static Region
        calculateBrowRegion(const std::vector<cv::Point2f> &points, const cv::Vec4f &line,
                            bool right);

        static Region
        calculateEyeRegion(const std::vector<cv::Point2f> &points, const cv::Vec4f &line,
                           bool right);

        static Region
        calculateLipsRegion(const std::vector<cv::Point2f> &points, const cv::Vec4f &line);

        static cv::Vec4f
        calcuateEyeRadius(const std::vector<cv::Point2f> &points, const cv::Vec4f &line,
                          bool right);
    ...
    public:
        ...
        detectFace(const cv::Mat &image, const std::string &tag, const std::string &classifier_dir);
        ...
        detectFaces(const cv::Mat &image, const std::string &tag,
         ...
         */
        static std::vector<std::vector<cv::Point2f>>
        detectFaces(cv::Size2i *size, const std::string &image_name,
                    const std::string &classifier_dir);
        ...
        static cv::Mat createMask(const std::vector<cv::Point2f> &points, float blur_radius = 0.0F,
                                  cv::Point2i *position = nullptr);

        static cv::Mat
        maskPolygon(const cv::Rect2i &rect, const std::vector<cv::Point2f> &points, int start,
                    int length);

        static cv::Mat maskPolygon(const cv::Rect2i &rect, const std::vector<cv::Point2f> &points);

        static cv::Mat
        maskPolygonSmooth(const cv::Rect2i &rect, const std::vector<cv::Point2f> &points,
                          const int level = 8);

        static std::vector<cv::Point2f>
        calculateBrowPolygon(const std::vector<cv::Point2f> &points, bool right);

        static std::vector<cv::Point2f>
        calculateEyePolygon(const std::vector<cv::Point2f> &points, bool right);

        static std::vector<cv::Point2f>
        calculateBlushPolygon(const std::vector<cv::Point2f> &points, bool right);

        static std::vector<cv::Point2f>
        calculateNosePolygon(const std::vector<cv::Point2f> &points);

        static std::vector<cv::Point2f>
        calculateLipPolygon(const std::vector<cv::Point2f> &points, bool upper);

        static std::vector<cv::Point2f>
        calculateTeethPolygon(const std::vector<cv::Point2f> &points);
        ...
        calculateIrisInfo(const std::vector<cv::Point2f> &points, bool right);

        static cv::RotatedRect
        calculateBlushRectangle(const std::vector<cv::Point2f> &points, bool right);
         ...
         */
        static cv::RotatedRect
        calculateBlushRectangle(const std::vector<cv::Point2f> &points, float angle, bool right);

        Region calculateBrowRegion(bool right) const {
            return calculateBrowRegion(points, line, right);
        }

        Region calculateEyeRegion(bool right) const {
            return calculateEyeRegion(points, line, right);
        }
        ...
        static cv::RotatedRect
        calculateRectangle(float angle, const cv::Point2f &left, const cv::Point2f &top,
         ...
         */
        static cv::Vec4f
        calcuateDistance(cv::Point2f &pivot, const cv::Point2f &left, const cv::Point2f &top,
                         const cv::Point2f &right, const cv::Point2f &bottom);

        cv::Vec4f calcuateEyeRadius(bool right) const {
            return calcuateEyeRadius(points, line, right);
        }
         ...
         */
        static cv::Mat
        maskSkinRegion(int width, int height, const std::vector<cv::Point2f> &points);
