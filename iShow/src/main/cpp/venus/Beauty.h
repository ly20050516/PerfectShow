...
namespace venus {
    ...
    public:
         ...
         */
        static void
        removeRedEye(cv::Mat &dst, const cv::Mat &src, const std::vector<cv::Point2f> &polygon,
        ...
        static void
        whitenSkinByLogCurve(cv::Mat &dst, const cv::Mat &src, const cv::Mat &mask, float level);

        static void
        beautifySkin(cv::Mat &dst, const cv::Mat &src, const cv::Mat &mask, float radius,
                     float level);
    };
