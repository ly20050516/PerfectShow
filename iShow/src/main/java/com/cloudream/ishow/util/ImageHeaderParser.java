...
public class ImageHeaderParser {
     ...
     * image type (or this is not an image) it will return a default value
     * rather than throwing an exception.
     *
     * @return The exif orientation if present or -1 if the header couldn't be
     * parsed or doesn't contain an orientation
     * @throws IOException
     */
    ...
    private int parseExifSegment(byte[] tempArray, int exifSegmentLength) throws IOException {
        ...
        if (read != exifSegmentLength) {
            if (Log.isLoggable(TAG, Log.DEBUG)) {
                Log.d(TAG, "Unable to read exif segment data" + ", length: " + exifSegmentLength + ", actually read: "
                        + read);
            }
            return UNKNOWN_ORIENTATION;
        }
    ...
    /**
     * Moves reader to the start of the exif segment and returns the length of
     * the exif segment or {@code -1} if no exif segment is found.
     */
    ...
    private static boolean handles(int imageMagicNumber) {
        return (imageMagicNumber & EXIF_MAGIC_NUMBER) == EXIF_MAGIC_NUMBER
                || imageMagicNumber == MOTOROLA_TIFF_MAGIC_NUMBER || imageMagicNumber == INTEL_TIFF_MAGIC_NUMBER;
    }
        ...
        public long skip(long total) throws IOException {
            ...
            while (toSkip > 0) {
                ...
                if (skipped > 0) {
                    ...
                    // Skip has no specific contract as to what happens when you
                    // reach the end of
                    // the stream. To differentiate between temporarily not
                    // having more data and
                    // having finished the stream, we read a single byte when we
                    // fail to skip any
                    // amount of data.
    ...
    public static void copyExif(ExifInterface originalExif, int width, int height, String imageOutputPath)
        ...
        String[] attributes = new String[]
                {ExifInterface.TAG_APERTURE, ExifInterface.TAG_DATETIME, ExifInterface.TAG_DATETIME_DIGITIZED,
                        ExifInterface.TAG_EXPOSURE_TIME, ExifInterface.TAG_FLASH, ExifInterface.TAG_FOCAL_LENGTH,
                        ExifInterface.TAG_GPS_ALTITUDE, ExifInterface.TAG_GPS_ALTITUDE_REF, ExifInterface.TAG_GPS_DATESTAMP,
                        ...
                        ExifInterface.TAG_GPS_TIMESTAMP, ExifInterface.TAG_ISO, ExifInterface.TAG_MAKE, ExifInterface.TAG_MODEL,
