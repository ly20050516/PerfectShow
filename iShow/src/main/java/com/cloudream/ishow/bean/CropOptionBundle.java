...
import com.cloudream.ishow.app.ImageCropActivity;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.Locale;

public class CropOptionBundle {
     ...
     */
    public CropOptionBundle setAspectRatio(int x, int y) {
        bundle.putInt(EXTRA_ASPECT_RATIO_X, x);
        bundle.putInt(EXTRA_ASPECT_RATIO_Y, y);
        return this;
    }
     ...
     */
    public CropOptionBundle useSourceImageAspectRatio() {
        bundle.putInt(EXTRA_ASPECT_RATIO_X, AspectRatio.DEFAULT_ASPECT_RATIO);
        bundle.putInt(EXTRA_ASPECT_RATIO_Y, AspectRatio.DEFAULT_ASPECT_RATIO);
        return this;
    }
     ...
     */
    public CropOptionBundle withMaxResultSize(@IntRange(from = 100) int width, @IntRange(from = 100) int height) {
        bundle.putInt(EXTRA_MAX_SIZE_X, width);
        bundle.putInt(EXTRA_MAX_SIZE_Y, height);
        return this;
    }
    ...
    public void setCompressionFormat(@NonNull Bitmap.CompressFormat format) {
        bundle.putString(EXTRA_COMPRESSION_FORMAT_NAME, format.name());
    }
    ...
    public void setCompressionQuality(@IntRange(from = 0, to = 100) int compressQuality) {
        bundle.putInt(EXTRA_COMPRESSION_QUALITY, compressQuality);
    }
    ...
    public void setAllowedGestures(@ImageCropActivity.GestureTypes int tabScale,
                                   @ImageCropActivity.GestureTypes int tabRotate, @ImageCropActivity.GestureTypes int tabAspectRatio) {
        bundle.putIntArray(EXTRA_ALLOWED_GESTURES, new int[]
    ...
    public void setMaxScaleMultiplier(@FloatRange(from = 1.0, fromInclusive = false) float maxScaleMultiplier) {
        bundle.putFloat(EXTRA_MAX_SCALE_MULTIPLIER, maxScaleMultiplier);
    }
    ...
    public void setImageToCropBoundsAnimDuration(@IntRange(from = 100) int durationMillis) {
        bundle.putInt(EXTRA_IMAGE_TO_CROP_BOUNDS_ANIM_DURATION, durationMillis);
    }
    ...
    public void setMaxBitmapSize(@IntRange(from = 100) int maxBitmapSize) {
        bundle.putInt(EXTRA_MAX_BITMAP_SIZE, maxBitmapSize);
    }
    ...
    public void setDimmedLayerColor(@ColorInt int color) {
        bundle.putInt(EXTRA_DIMMED_LAYER_COLOR, color);
    }
    ...
    public void setOvalDimmedLayer(boolean isOval) {
        bundle.putBoolean(EXTRA_OVAL_DIMMED_LAYER, isOval);
    }
    ...
    public void setShowCropFrame(boolean show) {
        bundle.putBoolean(EXTRA_SHOW_CROP_FRAME, show);
    }
    ...
    public void setCropFrameColor(@ColorInt int color) {
        bundle.putInt(EXTRA_CROP_FRAME_COLOR, color);
    }
    ...
    public void setCropFrameStrokeWidth(@IntRange(from = 0) int width) {
        bundle.putInt(EXTRA_CROP_FRAME_STROKE_WIDTH, width);
    }
    ...
    public void setShowCropGrid(boolean show) {
        bundle.putBoolean(EXTRA_SHOW_CROP_GRID, show);
    }
    ...
    public void setCropGridRowCount(@IntRange(from = 0) int count) {
        bundle.putInt(EXTRA_CROP_GRID_ROW_COUNT, count);
    }
    ...
    public void setCropGridColumnCount(@IntRange(from = 0) int count) {
        bundle.putInt(EXTRA_CROP_GRID_COLUMN_COUNT, count);
    }
    ...
    public void setCropGridColor(@ColorInt int color) {
        bundle.putInt(EXTRA_CROP_GRID_COLOR, color);
    }
    ...
    public void setCropGridStrokeWidth(@IntRange(from = 0) int width) {
        bundle.putInt(EXTRA_CROP_GRID_STROKE_WIDTH, width);
    }
    ...
    public void setToolbarColor(@ColorInt int color) {
        bundle.putInt(EXTRA_TOOL_BAR_COLOR, color);
    }
    ...
    public void setStatusBarColor(@ColorInt int color) {
        bundle.putInt(EXTRA_STATUS_BAR_COLOR, color);
    }

    /**
     * @param color - desired resolved color of the active and selected widget (default is
     *              orange) and progress wheel middle line
     */
    public void setActiveWidgetColor(@ColorInt int color) {
        bundle.putInt(EXTRA_UCROP_COLOR_WIDGET_ACTIVE, color);
    }
    ...
    public void setToolbarWidgetColor(@ColorInt int color) {
        bundle.putInt(EXTRA_UCROP_WIDGET_COLOR_TOOLBAR, color);
    }
    ...
    public void setToolbarTitle(@Nullable String text) {
        bundle.putString(EXTRA_UCROP_TITLE_TEXT_TOOLBAR, text);
    }
    ...
    public void setHideBottomControls(boolean hide) {
        bundle.putBoolean(EXTRA_HIDE_BOTTOM_CONTROLS, hide);
    }
    ...
    public void setFreeStyleCropEnabled(boolean enabled) {
        bundle.putBoolean(EXTRA_FREE_STYLE_CROP, enabled);
    }
    ...
    public void setAspectRatioOptions(int selectedByDefault, AspectRatio... aspectRatio) {
        ...
        bundle.putInt(EXTRA_ASPECT_RATIO_SELECTED_BY_DEFAULT, selectedByDefault);
        bundle.putParcelableArrayList(EXTRA_ASPECT_RATIO_OPTIONS,
