...
/**
 * This view is used for drawing the overlay on top of the image. It may have frame, crop guidelines
 * and dimmed area. This must have LAYER_TYPE_SOFTWARE to draw itself properly.
 */
public class OverlayView extends View {

    public interface OnOverlayViewChangeListener {
    ...
    private OnOverlayViewChangeListener mCallback;

    {
        mTouchPointThreshold = getResources()
                .getDimensionPixelSize(R.dimen.CropImageView_default_crop_rect_corner_touch_threshold);
        mCropRectMinSize = getResources().getDimensionPixelSize(R.dimen.CropImageView_default_crop_rect_min_size);
        mCropRectCornerTouchAreaLineLength = getResources()
                .getDimensionPixelSize(R.dimen.CropImageView_default_crop_rect_corner_touch_area_line_length);
    }
    ...
    public OnOverlayViewChangeListener getOverlayViewChangeListener() {
        return mCallback;
    }

    public void setOverlayViewChangeListener(OnOverlayViewChangeListener callback) {
        mCallback = callback;
    }
    ...
    public void setupCropBounds() {
        ...
        if (height > mThisHeight) {
            ...
            mCropViewRect.set(getPaddingLeft() + halfDiff, getPaddingTop(), getPaddingLeft() + width + halfDiff,
                    getPaddingTop() + mThisHeight);
        } else {
            int halfDiff = (mThisHeight - height) / 2;
            mCropViewRect.set(getPaddingLeft(), getPaddingTop() + halfDiff, getPaddingLeft() + mThisWidth,
    ...
    private void resizeCropViewRect(float touchX, float touchY) {
        ...
        mCropViewRect.set(changeWidth ? mTempRect.left : mCropViewRect.left,
                changeHeight ? mTempRect.top : mCropViewRect.top, changeWidth ? mTempRect.right : mCropViewRect.right,
    ...
    /**
     * * The order of the corners in the float array is: 0------->1 ^ | | | | v 3<-------2
     *
    ...
    private int getCurrentTouchCorner(float touchX, float touchY) {
        ...
        for (int i = 0; i < 8; i += 2) {
            double distanceToCorner = Math
                    .sqrt(Math.pow(touchX - mCropGridCorners[i], 2) + Math.pow(touchY - mCropGridCorners[i + 1], 2));
    ...
    protected void processStyledAttributes(@NonNull TypedArray a) {
        mOvalDimmedLayer = a.getBoolean(R.styleable.CropView_ovalDimmedLayer, DEFAULT_OVAL_DIMMED_LAYER);
        mDimmedColor = a.getColor(R.styleable.CropView_dimmedColor,
                getResources().getColor(R.color.CropImageView_default_dimmed));
        ...
        mShowCropFrame = a.getBoolean(R.styleable.CropView_showFrame, DEFAULT_SHOW_CROP_FRAME);

        initCropGridStyle(a);
        mShowCropGrid = a.getBoolean(R.styleable.CropView_showGrid, DEFAULT_SHOW_CROP_GRID);
    }
    ...
    private void initCropFrameStyle(@NonNull TypedArray a) {
        int cropFrameStrokeSize = a.getDimensionPixelSize(R.styleable.CropView_frameStrokeSize,
                getResources().getDimensionPixelSize(R.dimen.CropImageView_default_crop_frame_stoke_width));
        int cropFrameColor = a.getColor(R.styleable.CropView_frameColor,
                getResources().getColor(R.color.CropImageView_default_crop_frame));
    ...
    private void initCropGridStyle(@NonNull TypedArray a) {
        int cropGridStrokeSize = a.getDimensionPixelSize(R.styleable.CropView_gridStrokeSize,
                getResources().getDimensionPixelSize(R.dimen.CropImageView_default_crop_grid_stoke_width));
        int cropGridColor = a.getColor(R.styleable.CropView_gridColor,
                getResources().getColor(R.color.CropImageView_default_crop_grid));
        mCropGridPaint.setStrokeWidth(cropGridStrokeSize);
        mCropGridPaint.setColor(cropGridColor);

        mCropGridRowCount = a.getInt(R.styleable.CropView_gridRowCount, DEFAULT_CROP_GRID_ROW_COUNT);
        mCropGridColumnCount = a.getInt(R.styleable.CropView_gridColumnCount,
