...
import com.cloudream.ishow.bean.CropParameters;
import com.cloudream.ishow.bean.ImageState;
import com.cloudream.ishow.util.BitmapCropTask;
import com.cloudream.ishow.util.RectUtils;

import java.lang.ref.WeakReference;
import java.util.Arrays;

import static com.cloudream.ishow.bean.AspectRatio.DEFAULT_ASPECT_RATIO;
...
public class CropImageView extends TransformImageView {
    ...
    private OnCropBoundsChangeListener mCropBoundsChangeListener;
    ...
    public void cropAndSaveImage(@NonNull Bitmap.CompressFormat compressFormat, int compressQuality,
        ...
        final ImageState imageState = new ImageState(mCropRect, RectUtils.trapToRect(mCurrentImageCorners),
                getCurrentScale(), getCurrentAngle());

        final CropParameters cropParameters = new CropParameters(mMaxResultImageSizeX, mMaxResultImageSizeY,
                compressFormat, compressQuality, getImageInputPath(), getImageOutputPath(), getExifInfo());
    ...
    public void setTargetAspectRatio(float targetAspectRatio) {
        ...
        if (targetAspectRatio == DEFAULT_ASPECT_RATIO)
    ...
    public void setCropBoundsChangeListener(@Nullable OnCropBoundsChangeListener listener) {
        mCropBoundsChangeListener = listener;
    }
    ...
    public void postScale(float deltaScale, float px, float py) {
        ...
        if ((deltaScale > 1 && currentScale * deltaScale <= mMaxScale) ||
                (deltaScale < 1 && currentScale * deltaScale >= mMinScale))
     ...
     * {@link WrapCropBoundsRunnable} which animates image. Scale value must be calculated only if
     * image won't fill the crop bounds after it's translated to the crop bounds rectangle center.
     * Using temporary variables this method checks this case.
     */
    public void setImageToWrapCropBounds(boolean animate) {
        if (mBitmapLaidOut && !isImageWrapCropBounds()) {
            ...
            if (animate) {
                post(mWrapCropBoundsRunnable = new WrapCropBoundsRunnable(CropImageView.this,
                        mImageToWrapCropBoundsAnimDuration, currentX, currentY, deltaX, deltaY, currentScale,
     ...
     * calculate deltas between those rectangles sides. Third, depending on delta (its sign) put
     * them or zero inside an array. Fourth, using Matrix, rotate back those points (indents).
     *
    ...
    protected void onImageLaidOut() {
        ...
        if (mTargetAspectRatio == DEFAULT_ASPECT_RATIO) {
    ...
    protected void zoomImageToPosition(float scale, float centerX, float centerY, long durationMs) {
        ...
        post(mZoomImageToPositionRunnable = new ZoomImageToPosition(CropImageView.this, durationMs, oldScale,
                deltaScale, centerX, centerY));
    }
    ...
    /**
     * This method calculates initial image position so it is positioned properly. Then it sets
     * those values to the current image matrix.
     *
     * @param drawableWidth  - image width
     * @param drawableHeight - image height
     */
    ...
    /**
     * This method extracts all needed values from the styled attributes. Those are used to
     * configure the view.
     */
    @SuppressWarnings("deprecation")
    public void processStyledAttributes(@NonNull TypedArray a) {
        float targetAspectRatioX = Math
                .abs(a.getFloat(R.styleable.CropImageView_cropAspectRatioX, DEFAULT_ASPECT_RATIO));
        float targetAspectRatioY = Math
                .abs(a.getFloat(R.styleable.CropImageView_cropAspectRatioY, DEFAULT_ASPECT_RATIO));

        if (targetAspectRatioX == DEFAULT_ASPECT_RATIO || targetAspectRatioY == DEFAULT_ASPECT_RATIO)
            mTargetAspectRatio = DEFAULT_ASPECT_RATIO;
        else
     ...
     * are interpolated during the animation time. Runnable can be terminated either vie
     * {@link #cancelAllAnimations()} method or when certain conditions inside
    ...
    private static class WrapCropBoundsRunnable implements Runnable {
        ...
        public WrapCropBoundsRunnable(CropImageView cropImageView, long durationMs, float oldX, float oldY,
                                      float centerDiffX, float centerDiffY, float oldScale, float deltaScale,
     ...
     * animation time. Runnable can be terminated either vie {@link #cancelAllAnimations()} method
    ...
    private static class ZoomImageToPosition implements Runnable {
        ...
        public ZoomImageToPosition(CropImageView cropImageView, long durationMs, float oldScale, float deltaScale,
