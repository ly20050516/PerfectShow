...
public abstract class ImageViewTouchBase extends ImageView {
    ...
    protected final float[] mMatrixValues = new float[9];//{ 1,0,0, 0,1,0, 0,0,1 };
    ...
    public void setScaleType(ScaleType scaleType) {
        if (scaleType == ScaleType.MATRIX)
            ...
            Log.w(TAG, "Unsupported scale type. Only MATRIX can be used");
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (LOG_ENABLED) {
            Log.d(TAG, "onLayout: " + changed + ", bitmapChanged: " + mBitmapChanged + ", scaleChanged: "
                    + mScaleTypeChanged);
        }
    ...
    public void setImageBitmap(final Bitmap bitmap) {
        setImageBitmap(bitmap, getDisplayMatrix(), ZOOM_INVALID, ZOOM_INVALID);
    }

    public void setImageBitmap(final Bitmap bitmap, Matrix matrix) {
        setImageBitmap(bitmap, matrix, ZOOM_INVALID, ZOOM_INVALID);
    }
    ...
    public void setImageBitmap(final Bitmap bitmap, Matrix matrix, float min_zoom, float max_zoom) {
        ...
        setImageDrawable((bitmap != null) ? drawable : null, matrix, min_zoom, max_zoom);
    }
    ...
    protected void scrollBy(final float dx, final float dy, final float durationMs) {
