...
public class GPUImageToneCurveFilter extends GPUImageFilter {
    private static final String TONE_CURVE_FRAGMENT_SHADER =
    ...
    public GPUImageToneCurveFilter() {
        super(GPUImageFilterType.TONE_CURVE, NO_FILTER_VERTEX_SHADER, TONE_CURVE_FRAGMENT_SHADER);
    ...
    public void setFromCurveFileInputStream(InputStream input) {
        try {
            ...
            for (int i = 0; i < totalCurves; i++) {
                ...
                // Curves dialog graph) and the second is the input value. All coordinates have
                // range 0 to 255.
                ...
                if ((mRedCurve.size() >= 256) && (mGreenCurve.size() >= 256) && (mBlueCurve.size() >= 256)
                    ...
                    for (int currentCurveIndex = 0; currentCurveIndex < 256; currentCurveIndex++) {
                        ...
                        toneCurveByteArray[currentCurveIndex * 4 + 2] = (byte) ((int) Math
                                .min(Math.max(currentCurveIndex + mBlueCurve.get(currentCurveIndex)
                                        + mRgbCompositeCurve.get(currentCurveIndex), 0), 255)
                                & 0xff);
                        toneCurveByteArray[currentCurveIndex * 4 + 1] = (byte) ((int) Math
                                .min(Math.max(currentCurveIndex + mGreenCurve.get(currentCurveIndex)
                                        + mRgbCompositeCurve.get(currentCurveIndex), 0), 255)
                                & 0xff);
                        toneCurveByteArray[currentCurveIndex * 4] = (byte) ((int) Math.min(Math.max(currentCurveIndex
                                + mRedCurve.get(currentCurveIndex) + mRgbCompositeCurve.get(currentCurveIndex), 0), 255)
                                & 0xff);
    ...
    private ArrayList<Float> createSplineCurve(PointF[] points) {
        ...
        for (Point newPoint : splinePoints) {
            ...
            float distance = (float) Math
                    .sqrt(Math.pow((origPoint.x - newPoint.x), 2.0) + Math.pow((origPoint.y - newPoint.y), 2.0));
    ...
    private ArrayList<Point> createSplineCurve2(Point[] points) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            for (int x = cur.x; x < next.x; x++) {
                ...
                double y = a * cur.y + b * next.y
                        + (h * h / 6) * ((a * a * a - a) * sd[i] + (b * b * b - b) * sd[i + 1]);
