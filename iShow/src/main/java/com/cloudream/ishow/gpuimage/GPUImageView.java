...
public class GPUImageView extends FrameLayout {
     ...
     * folder on the phone below the given folderName and fileName. <br>
    ...
    public Bitmap capture(final int width, final int height) throws InterruptedException {
        ...
        mForceSize = new Point(width, height);
    ...
    public Bitmap capture() throws InterruptedException {
        ...
        final int[] pixelMirroredArray = new int[count];
        mGPUImage.runOnGLThread(new Runnable() {
            @Override
            public void run() {
                final IntBuffer pixelBuffer = IntBuffer.allocate(count);
                GLES20.glReadPixels(0, 0, width, height, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, pixelBuffer);
                int[] pixelArray = pixelBuffer.array();

                // Convert upside down mirror-reversed image to right-side up normal image.
                final int tmp[] = new int[width];
                for (int i = 0; i < height; ++i) {
                    int top_index = i * width;
                    int bottom_index = count - width - top_index;
                    System.arraycopy(pixelArray, top_index, tmp, 0, width);
                    System.arraycopy(pixelMirroredArray, bottom_index, pixelArray, top_index, width);
                    System.arraycopy(tmp, 0, pixelMirroredArray, bottom_index, width);
...
//					}
    ...
    private class GPUImageGLSurfaceView extends GLSurfaceView {
        ...
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            if (mForceSize != null) {
                super.onMeasure(MeasureSpec.makeMeasureSpec(mForceSize.x, MeasureSpec.EXACTLY),
                        MeasureSpec.makeMeasureSpec(mForceSize.y, MeasureSpec.EXACTLY));
            } else {
                super.onMeasure(widthMeasureSpec, heightMeasureSpec);
            }
        }
        ...
        private void saveImage(final String folderName, final String fileName, final Bitmap image) {
            ...
            try {
                ...
                MediaScannerConnection.scanFile(getContext(), new String[]
                        {file.toString()}, null, new MediaScannerConnection.OnScanCompletedListener() {
                    @Override
