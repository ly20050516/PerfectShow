...
public class GPUImageTransformFilter extends GPUImageFilter {
    private static final String TRANSFORM_VERTEX_SHADER =
    ...
    // This applies the transform to the raw frame data if set to YES, the default of NO takes the
    // aspect ratio of the image input into account when rotating
    ...
    public GPUImageTransformFilter() {
        super(GPUImageFilterType.TRANSFORM2D, TRANSFORM_VERTEX_SHADER, NO_FILTER_FRAGMENT_SHADER);
    ...
    public void onOutputSizeChanged(final int width, final int height) {
        ...
        if (!ignoreAspectRatio) {
            Matrix.orthoM(orthographicMatrix, 0, -1.0F, 1.0F, -1.0F * (float) height / (float) width,
    ...
    public void onDraw(final int textureId, final FloatBuffer cubeBuffer, final FloatBuffer textureBuffer) {
        ...
        if (!ignoreAspectRatio) {
            ...
            vertBuffer = ByteBuffer.allocateDirect(adjustedVertices.length * 4).order(ByteOrder.nativeOrder())
                    .asFloatBuffer();
